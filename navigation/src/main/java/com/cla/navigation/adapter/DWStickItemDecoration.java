...
public abstract class DWStickItemDecoration extends RecyclerView.ItemDecoration {
     ...
     */
    public DWStickItemDecoration(Context context, int labelHeight, int labelColor, int textSize,
            int textColor) {
        mContext = context;
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //小于0时才需要绘制
            if (i == 0 && (position > 0)
                    || (position == 0 && (child.getTop() - mLabelHeight) < 0)) {
                ...
                String text = getItemLabelStr(position);

                Rect textRect = new Rect();
                mPaint.getTextBounds(text, 0, text.length(), textRect);
                int textHeight = textRect.height();//文字高
                y += textHeight / 2;

                int x = (int) (mRect.width() / 2 - mPaint.measureText(text) / 2);
